...
void MultiWiFi::startPortal(const char *ssid, const char *password) {
    ...
    portal.on("/generate_204", HTTP_GET, [this]() {
        portal.sendHeader("Location", "/");
        portal.send(302);
    });
    portal.on(UriGlob("/*"), HTTP_ANY, [this]() {
        portal.sendHeader("Location", "/");
        portal.send(302);
    });
    portal.begin();
...
void MultiWiFi::sendPortalPage() {
    ...
    WiFi.scanDelete();
    page +=
            "</tbody></table><p><label>Password<br><input name=password type=password autocomplete=current-password></label></p>"
