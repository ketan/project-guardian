...
- Use a captive portal for initial Wi-Fi provisioning, then expose the normal admin UI/API once the device is on the configured network.
...
- If Wi-Fi is not configured or the device cannot reconnect, fall back to AP + captive portal for local recovery.
...
- Integration-style tests should verify captive-portal onboarding, Wi-Fi recovery, and the transition from AP setup to normal networked operation.
...
- Initial local onboarding uses a captive portal instead of the normal admin frontend.
